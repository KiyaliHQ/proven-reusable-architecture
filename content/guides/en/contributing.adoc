...
Submit a *Transversal PRA* if your pattern:
...
*Note*: Most PRAs start as Domain PRAs. Transversal PRAs typically come from:

* Cross-cutting teams (Software Engineering, Security, etc.) - /guides/08-governance#-flow-2-top-down-cross-cutting-teams--transversal[*Flow 2*]
* Domain PRAs promoted to Transversal - /guides/08-governance#-flow-1-organic-domain--transversal[*Flow 1*]
...
* *Transversal PRA*: `content/pras/{lang}/transversal/{category}/operationalizing/pra-name.md`
* *By Domain PRA*: `content/pras/{lang}/by-domain/{domain}/{category}/operationalizing/pra-name.md`

Where:

* `{lang}` = `fr` or `en`
* `{category}` = PRA category (e.g., `ctp`, `software-engineering`, `pratique-architecture` for Transversal; plus `application`, `business`, `data`, `integration`, `security`, `technology` for By Domain)
* `{domain}` = `particuliers`, `entreprises`, or `gestion-patrimoine` (if by-domain)

==== Examples

*Transversal CTP PRA* (applicable to all sectors):

[source,bash]
----
content/pras/fr/transversal/ctp/operationalizing/api-gateway-pattern.md
content/pras/en/transversal/ctp/operationalizing/api-gateway-pattern.md
----

*By Domain Security PRA* (Retail):

[source,bash]
----
content/pras/fr/by-domain/particuliers/security/operationalizing/kyc-verification.md
content/pras/en/by-domain/particuliers/security/operationalizing/kyc-verification.md
----
...
git commit -m "feat: add PRA - PRA Name (Transversal Tech Candidate)"
...
. *Assign reviewers*
* *Transversal* -> `@KiyaliHQ/comite-architectes-experts`
* *By Domain Particuliers* -> `@KiyaliHQ/comite-gov-particuliers`
* *By Domain Entreprises* -> `@KiyaliHQ/comite-gov-entreprises`
* *By Domain Patrimoine* -> `@KiyaliHQ/comite-gov-patrimoine`

. *Post status comment*
* Validation checklist
* Next steps
* Timeline (2-4 weeks for Transversal, 5-10 days for By Domain)
...
*For By Domain PRAs* (Retail, Corporate, Wealth Management):
...
*For Transversal PRAs* (Cross-cutting):
...
=== Transversal PRA: Candidate -> Approved

For a Transversal Candidate PRA to become *Transversal Approved*, it must:
