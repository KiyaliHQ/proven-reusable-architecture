...
Soumettez un *PRA Transversale* si votre patron :
...
*Note* : La plupart des PRAs commencent comme PRAs Domaine. Les PRAs Transversale proviennent typiquement de :

* Équipes transversales (Software Engineering, Sécurité, etc.) - /guides/08-governance#-flow-2--top-down-équipes-transversales---transversale[*Flow 2*]
* PRAs Domaine promus vers Transversale - /guides/08-governance#-flow-1--organique-domaine---transversale[*Flow 1*]
...
* *PRA Transversale* : `content/pras/{lang}/transversale/{category}/operationalizing/nom-du-pra.md`
* *PRA Par Domaine* : `content/pras/{lang}/par-domaine/{domain}/{category}/operationalizing/nom-du-pra.md`

Où :

* `{lang}` = `fr` ou `en`
* `{category}` = catégorie du PRA (ex: `ctp`, `software-engineering`, `pratique-architecture` pour Transversale; plus `application`, `business`, `data`, `integration`, `security`, `technology` pour Par Domaine)
* `{domain}` = `particuliers`, `entreprises`, ou `gestion-patrimoine` (si par-domaine)

==== Exemples

*PRA Transversale CTP* (applicable à tous les secteurs) :

[source,bash]
----
content/pras/fr/transversale/ctp/operationalizing/api-gateway-pattern.md
content/pras/en/transversale/ctp/operationalizing/api-gateway-pattern.md
----

*PRA Par Domaine Security* (Particuliers) :

[source,bash]
----
content/pras/fr/par-domaine/particuliers/security/operationalizing/kyc-verification.md
content/pras/en/par-domaine/particuliers/security/operationalizing/kyc-verification.md
----
...
git commit -m "feat: add PRA - Nom du PRA (Transversale Tech Candidate)"
...
* *Transversale* -> `@KiyaliHQ/comite-architectes-experts`
* *Par Domaine Particuliers* -> `@KiyaliHQ/comite-gov-particuliers`
* *Par Domaine Entreprises* -> `@KiyaliHQ/comite-gov-entreprises`
* *Par Domaine Patrimoine* -> `@KiyaliHQ/comite-gov-patrimoine`

. *Poster un commentaire de statut*
* Checklist de validation
* Prochaines étapes
* Timeline (2-4 semaines pour Transversale, 5-10 jours pour Par Domaine)
...
*Pour PRAs Par Domaine* (Particuliers, Entreprises, Gestion de Patrimoine) :
...
*Pour PRAs Transversale* (Transversal) :
...
=== PRA Transversale : Candidate -> Approved

Pour qu'un PRA Transversale Candidate devienne *Transversale Approved*, il doit :
